...
The **Server** executes local training rounds when instructed by the orchestrator.
This module implements the gRPC **Server** service.
...
Initializes the server with run configuration and (optional) privacy settings
before any rounds start.
...
+----------------------+--------+-----------------------------------------------+
| ``received_timestamp`` | int64  | Server-side Unix seconds when processed.    |
+----------------------+--------+-----------------------------------------------+
...
Starts a local training round using a global reference (e.g., base weights or
adapter) and round-specific knobs.
...
Servers construct these after training; see orchestrator docs for response.
...
* If DP is enabled, update privacy accountant per step and stop when ``ε``
  exceeds target.
